...
def insert_new_testresult(conn, matr_nr, vst_nr, persNr_prof, result):
    ...
    d = cur.fetchall()
    result_check = ''.join(i for i in str(result) if i not in special_character_for_result)
...
def insert_new_student_to_event(conn, matr_nr, vst_nr):
    ...
    if not a:
        ...
        cur.execute("""SELECT VstNr FROM pruefen
                    WHERE Note > 4.0 AND MatrNr = {} AND VstNr = {} """.format(matr_nr, vorgaenger[0][0]))
        note = cur.fetchall()
        if note:
            error = "Nicht möglich, da die Vorgänger-Veranstaltung nicht erfolgreich (Note > 4.0) absolviert wurde."
            print(error)
            return error
        else:
